...
1. **Install the package.**
   ...
   PrimeReact and its icons (`primereact`, `primeicons`), plus `pixi.js` (`PivotViewer`), `framer-motion` (animated panels), and `allotment` (`DataPage`), are **bundled dependencies** — they install automatically with the package, so there's nothing extra to add. The real peers — `react`/`react-dom` 19+, `@cratis/arc*`, `reflect-metadata`, and `tsyringe` — already come with the Arc frontend from the prerequisites.
