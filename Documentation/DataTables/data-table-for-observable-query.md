...
While the first observable result is performing without rows, the table shows a loading row with a status announcement instead of `emptyMessage`. During a refetch with rows, it keeps the rows and marks the table busy. An unauthorized result takes precedence over a failed result; either displays a failure row with an alert instead of an empty state. A successful empty result still shows `emptyMessage`. Set `loadingMessage`, `failureMessage`, or `unauthorizedMessage` to customize these states. See [Loading, empty, and failed queries](data-table-for-query.md#loading-empty-and-failed-queries) for the state rules and defaults.
