...
`content` accepts a React node, including text, elements, and the number `0`. `undefined`, `null`, `false`, and an empty string disable the tooltip. Keep tooltip content non-interactive: an ARIA tooltip is not focusable, so links and buttons inside it cannot be used reliably. Use a popover for interactive content. `Button` and `IconButton` retain their text-only `tooltip` prop.
