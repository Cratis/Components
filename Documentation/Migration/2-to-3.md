...
This historical guide is preserved for Components 3 consumers. Links to version-specific Components pages use the immutable Components 3 merge commit `080a441` so they cannot drift to Components 4 behavior. An application still on 2.x does not have to pass through Components 3: [Migrate directly from Components 2](3-to-4.md#migrate-directly-from-components-2) covers the move straight to Components 4, which has no PrimeReact dependency.
...
> [!TIP]
> This guide covers what changes **for `@cratis/components` consumers specifically** — peer
> dependencies, import paths, the Cratis wrappers, styling, licensing. For the underlying
> PrimeReact 10 → 11 changes themselves (including areas this library doesn't wrap), also read
> PrimeReact's own [Updating to v11](https://primereact.dev/docs/primitive/guides/migration/updating-to-v11)
> guide.
...
- `primereact` pins `@primereact/core`, `@primereact/headless` and `@primereact/hooks` to the same release. Declaring them anyway is what makes a strict installer (pnpm, Yarn PnP) resolve them for the library too; use the peer ranges published by the selected Components 3 package rather than copying a patch version from a guide.
...
- `@primereact/styles` and `@primeuix/themes` are **optional** peers, declared from Components 3.4 (3.5 also added `@primereact/hooks` as a required peer). Install them only for PrimeReact's styled mode — see [Theming without a theme stylesheet](#theming-without-a-theme-stylesheet). The baseline theme and unstyled mode need neither.
...
### Keep Arc packages aligned

The Arc peer window changed across Components 3 patch and minor releases. Read `peerDependencies` from the Components 3 version selected by the application's lockfile instead of treating this historical guide as an Arc compatibility matrix.

> [!WARNING]
> **Keep `@cratis/arc`, `@cratis/arc.react` and `@cratis/arc.vite` on one mutually compatible release line.** `@cratis/arc.react` pins its Core dependency, so drift can install a second copy. `ObservableQuerySubscription` has a private field and is nominally typed; two copies can produce a _"types have separate declarations of a private property `_connection`"_ diagnostic in code touching an observable query. This is an Arc packaging boundary, not a Components one; follow the selected Arc packages' own manifests.
...
- **`@cratis/components/tokens` is not optional** — every component stylesheet reads its colors from the `--cratis-*` variables it defines, `var()`-with-no-literal-fallback. Skip this import (as this library's own Storybook did, briefly) and cards, panels and overlays that reference `--cratis-*` render with no background, no border and no text color — not an obviously "missing" look, but a blank one.
...
| `primereact/calendar`, `primereact/inputtextarea`, `primereact/multiselect`, `primereact/chips`, `primereact/colorpicker` | consume through the [CommandForm fields](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/CommandForm/index.md) (`CalendarField`, `TextAreaField`, `MultiSelectField`, `ChipsField`, `ColorPickerField`) |
| `import { PrimeReactProvider } from 'primereact/api'`                                                                     | `import { PrimeReactProvider } from '@primereact/core'` — or just use [`CratisComponentsProvider`](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/Common/cratis-components-provider.md)                |
...
Where the library needed a removed module, it now owns a replacement: a Cratis action toolbar (for `menubar`, driven by the same `model` array shape and reached through `<DataPage.MenuItems>`), [`Column`](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/DataTables/column-configuration.md) plus its filter menu (for `column`), [`StepperPanel`](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/StepperCommandDialog/index.md) (for `stepperpanel`), and `MultiSelectField` re-expressed over our own [`Dropdown`](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/Dropdown/index.md) wrapper (v11's `Select` has no `multiple` prop of the v10 shape).
...
Pick one of three paths — the [Styling](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/Styling/index.md) section walks each one:

**A. [The Cratis baseline theme](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/Styling/baseline-theme.md).** Cratis-authored MIT CSS that assigns the `--cratis-*` tokens directly, light and dark, for a polished default with no preset and no extra dependency. It defers to a preset's `--p-*` values when one is present.
...
> [!CAUTION]
> **A `pt` preset must position every overlay part it touches, not just color it.** PrimeReact 11's headless `Dialog` renders its `backdrop` and `positioner` as plain sibling `<div>`s and leaves placement entirely to CSS — there is no built-in centering. A `pt` preset that only sets background/border classes on `popup`/`header`/`footer` and skips `positioner`/`backdrop` compiles fine and looks fine in isolation, but the dialog renders as a zero-height block wherever it falls in normal document flow — typically far below the fold, looking exactly like the "Open" button silently does nothing. Give both parts `position: fixed; inset: 0;` (plus `display: flex; align-items: center; justify-content: center;` on `positioner`) — see [the baseline theme's Dialog rules](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/Styling/baseline-theme.md) for the reference CSS if you're building a `pt` preset from scratch.
...
> [!CAUTION]
> **`light-dark()` resolves in the DOM, not in a detached `<canvas>`.** If your own code reads one of these variables with `getComputedStyle(...).getPropertyValue('--surface-b')` and feeds the raw string into a `CanvasRenderingContext2D.fillStyle` (a common pattern for canvas/WebGL-rendered UI), the assignment silently fails — `fillStyle` cannot parse `light-dark(...)` and is left at its default (opaque black), with no exception and no console warning. Custom properties substitute `var()` textually but do not resolve `light-dark()` down to a used color the way an actual rendered element does. Resolve through a real, attached DOM element instead: set `.style.color` on it and read back `getComputedStyle(el).color`, which always comes back as `rgb()`/`rgba()`.
...
[`Dialog`](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/Dialogs/dialog.md) / [`CommandDialog`](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/CommandDialog/index.md) / [`StepperCommandDialog`](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/StepperCommandDialog/index.md) keep their public APIs, including `initialFocus` (`DialogInitialFocus.Confirm | Cancel | Content`), which is re-expressed on v11's focus trap and behaves identically.
...
**`useOverlayZIndex` is removed.** PrimeReact 11 does both natively: `Select.Portal` defaults to `appendTo: 'body'`, and the shared z-index registry gives a later-opened overlay a value above whatever is already registered. Components 3 regression coverage verifies that a select opened inside a dialog is portaled to `document.body` above that dialog; the guide does not pin those runtime-assigned z-index values to one PrimeReact patch.
...
v11 is unstyled-first: outside [styled mode](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/Styling/themed.md), PrimeReact elements carry **no `p-*` class at all**. Parts are identified by data attributes instead — `[data-scope="dialog"][data-part="close"]`, `[data-scope="select"][data-part="trigger"]`, and so on. Selectors written against v10 class names will silently match nothing there. In styled mode the `p-*` class names are back — `styledMode()` applies PrimeReact's component styles to every primitive — and the theme sits in the `primereact` cascade layer, so a plain `.p-button { … }` in your own CSS overrides it as it did on v10. (`pt` slot keys are unaffected either way.)

## What's new (nothing to migrate — just available)

- **[Notifications](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/Notifications/index.md)** — `Toaster`, the imperative `toast`, and `toastCommandResult(result)` to surface an Arc command result as the right toast (validation → per-field messages, exceptions → generic, never stack traces).
- **[Display](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/Display/index.md)** — `Tag`, `Badge`, `Chip`, `Skeleton`, `Avatar`, `ProgressBar`.
- **CommandForm fields** — [PasswordField](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/CommandForm/password-field.md), [ToggleSwitchField](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/CommandForm/toggle-switch-field.md), [RatingField](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/CommandForm/rating-field.md).
- **[AutoCommandForm](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/CommandForm/auto-command-form.md)** — generates a `CommandForm`'s fields from the command's own `propertyDescriptors`, with a `registerFieldTypeProvider` registry for custom types.
- **[`@cratis/components/theme`](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/Styling/baseline-theme.md)** — the Cratis baseline theme (MIT CSS). It now positions the dialog backdrop and positioner itself; previously a baseline-theme dialog could render below the page content.
- **[`@cratis/components/styled`](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/Styling/themed.md)** — `styledMode()`, `CratisPreset`, `primeReactStyles`, `primeReactCssLayer`, `primeReactCssLayerOrder` and `cratisDarkModeSelector`: PrimeReact's styled mode, wired for the provider.
...
What the styling choice changes is whether you additionally depend on `@primereact/styles` and `@primeuix/themes` — both PrimeUI-licensed too, and needed only for styled mode. [The Cratis baseline theme](https://github.com/Cratis/Components/blob/080a441080b44da5f6dc42209b296b1faa7d4708/Documentation/Styling/baseline-theme.md) is Cratis-authored MIT CSS embedding no PrimeTek values, so that _stylesheet_ carries no PrimeTek terms — but rendering it still runs PrimeReact 11, which needs a key.

> [!TIP]
> **In Storybook or another Vite-based dev tool**, export the key as a plain `PRIMEUI_LICENSE`
> environment variable — matching the name PrimeReact's own docs and CLI use — rather than
> inventing a tool-specific name. Vite's dev server only forwards `VITE_`/tool-prefixed env vars to
> `import.meta.env` by default, so a dev-tool config still needs to relay a plain-named var
> explicitly (for example via `define: { 'import.meta.env.PRIMEUI_LICENSE': JSON.stringify(process.env.PRIMEUI_LICENSE) }`
> in a Vite config) rather than silently dropping it — this library's own `Source/.storybook/main.ts`
> does exactly that.
...
**At the 3.0 release, `@cratis/components` 2.x stayed on PrimeReact 10 and fully MIT.** It was not getting new features, but it was the supported way to remain MIT-only.

Components 4 later removed PrimeReact from `@cratis/components` altogether; PrimeReact appears only in the optional renderer adapter packages. To leave PrimeUI licensing today, move to Components 4 with the [3.x → 4.0 guide](3-to-4.md).

> Nothing here is legal advice, and this summary may lag PrimeTek's terms. The authoritative text is the `LICENSE.md` inside the `primereact` package, [PrimeReact's own migration guide](https://primereact.dev/docs/primitive/guides/migration/updating-to-v11), and the license pages linked above.
