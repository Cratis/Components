...
† The removed root `CommandStepper` namespace aliased the _entire_ `CommandDialog` module. The codemod therefore maps that namespace to `@cratis/components/CommandDialog`, preserving every existing namespace member — not only `CommandStepper.StepperCommandDialog`, `CommandStepper.CommandDialog`, and `CommandStepper.CommandStepper`, but also the module's other exports such as `CommandStepper.StepperPanel` and `CommandStepper.applyBeforeExecute`. New code that needs only the standalone component should use the narrower named import from `@cratis/components/CommandStepper`.
