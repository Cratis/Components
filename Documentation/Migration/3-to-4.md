...
The package retains its Arc peer range of `>=20.3.1 <23` and Fundamentals range of `^7.10.3`. When using `AutoCommandForm`, update `@cratis/arc` and `@cratis/arc.react` together to 22.16.0 or newer: generated forms rely on that version honoring an explicit field name rather than inferring every field from the same dynamic accessor.
...
:::caution[CommandForm field `onChange` is not a typed prop]
Components 4 CommandForm fields such as `InputTextField`, `CheckboxField`, `DropdownField`, and `MultiSelectField` are Arc `asCommandFormField` wrappers whose public props do not accept `onChange`. Rewriting only the callback leaves an invalid attribute and causes TS2322. The codemod leaves these field handlers unchanged and reports a diagnostic. Move the side effect to the enclosing `CommandForm`'s `onFieldChange(command, fieldName, oldValue, newValue, validationInfo?)` callback. Legacy field `onValueChange` callbacks with a proven event payload are rewritten to semantic values; already-semantic ones are untouched. The standalone `Dropdown` declares `onChange` and can be rewritten.
:::
...
6. Keep the application-owned grouped/lazy table adapter. `DataTableCore` does not claim to provide grouping or controlled server sorting, and the proposed renderer-neutral table state seam ([#109](https://github.com/Cratis/Components/issues/109)) was closed as not planned.
...
- `Column` remains the declarative column marker. Components 4.0 through 4.13 typed only `selectionMode='single'`, because the old `'multiple'` value advertised checkbox behavior that the implementation never provided. Components 4.14.0 restores `'multiple'` with working behavior: set `selectionMode='multiple'` on the table or `DataPage`, add a `<Column selectionMode='multiple' />`, and track the set through `selectedItems` and `onSelectedItemsChange`. Code written against the Components 3 `'multiple'` type must adopt those props; see [Column Configuration: Selection](../DataTables/column-configuration.md#selection).
