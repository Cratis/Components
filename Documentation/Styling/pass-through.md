...
| ------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| `ButtonParts` / `IconButtonParts`           | `root`, `icon`, `label`, `spinner`                                                                                                                                                     | button, spans                                                 |
| `TextInputParts` / `TextAreaParts`           | `root`                                                                                                                                                                                 | native input or textarea                                      |
| `NumberInputParts`                            | `root`, `group`, `input`, `prefix`, `suffix`, `step`, `description`, `error`                                                                                                         | owned number-field composition                                |
| `CheckboxParts` / `RadioParts`               | `root`, `input`, `box`, `indicator`, `label`                                                                                                                                           | label, native input, spans                                    |
| `SwitchParts`                                | `root`, `input`, `control`, `handle`, `label`                                                                                                                                          | label, native input, spans                                    |
| `SurfaceParts`                               | `root`                                                                                                                                                                                 | div, section, or article                                     |
| `DialogParts`                               | `backdrop`, `positioner`, `root`, `header`, `title`, `subtitle`, `close`, `content`, `footer`, `confirm`, `cancel`                                                                                 | divs/header/heading/footer/buttons                            |
| `ToggleGroupParts`                          | `root`, `option`, `icon`, `label`                                                                                                                                                                                     | group/buttons                                                 |
| `TabsParts`                                 | `root`, `list`, `tab`, `panel`                                                                                                                                                                                        | divs/tabs/panels                                              |
| `TagGroupParts`                             | `root`, `list`, `tag`, `remove`, `input`                                                                                                                                                                              | div/list/tags/buttons/input                                   |
| `BreadcrumbsParts`                          | `root`, `item`, `link`, `separator`                                                                                                                                                                                   | nav/ol/li/links                                               |
...
| `ComboBoxParts`                             | `root`, `input`, `trigger`, `popover`, `listbox`, `option`, `optionLabel`, `optionDescription`, `loading`, `empty`, `failure`, `action`, `description`, `error`                         | wrapper, input, button, popover/listbox/options, status text  |
...
| `DataTableParts`                            | `root`, `search`, `searchInput`, `tableContainer`, `table`, `head`, `headerRow`, `headerCell`, `body`, `row`, `cell`, `emptyRow`, `emptyCell`, `loadingRow`, `loadingCell`, `failureRow`, `failureCell`                                          | div/input/table sections/rows/cells                           |
...
| `ChatSidebarParts`                          | `backdrop`, `root`, `header`, `title`, `back`, `close`, `content`                                                                                                                      | backdrop/panel divs, header, heading, buttons                 |
| `EventsViewParts`                           | `timeline`, `event`, `separator`, `marker`, `connector`, `content`                                                                                                                     | timeline divs and event elements                              |
...
| Basic controls | text inputs and surfaces use `root`; NumberInput uses `root`, `group`, `input`, `prefix`, `suffix`, `step`, `description`, `error`; choices use `root`, `input`, `label`, plus `box`/`indicator` or `control`/`handle`                                                                                                                              |
| Dialog         | `backdrop`, `positioner`, `root`, `header`, `title`, `subtitle`, `close`, `content`, `busy-scope`, `footer`, `confirm`, `cancel`                                                                                                                                                                                                   |
| ComboBox       | `root`, `input`, `trigger`, `popover`, `listbox`, `option`, `optionLabel`, `optionDescription`, `loading`, `empty`, `failure`, `action`, `description`, `error`                                                                                                                                                         |
...
| DataTable      | `root`, `search`, `search-input`, `table-container`, `table`, `head`, `header-row`, `header-cell`, `header-content`, `sort`, `select-all`, `filter-trigger`, `filter-popover`, `filter-menu`, `filter-actions`, `body`, `row`, `cell`, `empty-row`, `empty-cell`, `loading-row`, `loading-cell`, `failure-row`, `failure-cell`                                                       |
...
| Command fields | `root`, plus the typed field-specific values listed above (`input`, `textarea`, `group`, `box`, `control`, `handle`, `indicator`, `option`, `item`, `remove`, `value`, `star`, `toggle`)                                                                                                                               |
| Display        | every component has `root`; Avatar adds `image`, `fallback`; Chip adds `icon`, `label`, `remove`; Message adds `icon`, `text`; ProgressBar adds `indicator`, `label`; ProgressSpinner adds `svg`, `track`, `range`; Tag adds `icon`, `label`                                                                            |
| Chat sidebar   | `backdrop`, `root`, `header`, `title`, `back`, `close`, `content`                                                                                                                                                                                                                                                      |
...
`busy-scope` wraps the Dialog content and footer in a `fieldset` that is disabled while the dialog is busy; it has no `DialogParts` key. ComboBox is the one surface whose multi-word DOM values keep the camel-case spelling of its keys (`optionLabel`, `optionDescription`); match them exactly.

The DataTable `root` exposes `data-busy` and the native `table` exposes `aria-busy="true"` during a refetch with rows. A failure row exposes `data-reason="failed"` or `"unauthorized"`; loading and failure messages have an inner status or alert element, not a role on the row.

`header-content`, `sort`, `select-all` (the multiple-selection header checkbox), and `filter-trigger` are DOM targets but are not `DataTableParts` keys. Customize them with CSS rather than claiming a nonexistent typed key.
...
| `data-placement`   | Dialog viewport placement: `center`, `start`, `end`. |
