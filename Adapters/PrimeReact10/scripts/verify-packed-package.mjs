...
try {
    ...
    if (JSON.stringify(packageJson.cratis?.license) !== JSON.stringify(expectedLicense)) {
        throw new Error('Packed static metadata does not preserve the MIT policy.');
    }
    ...
    link(
        path.join(packageDirectory, 'node_modules/primereact'),
        path.join(nodeModules, 'primereact'),
    );

    writeFileSync(
        path.join(temporary, 'runtime.mjs'),
        "const api = await import('@cratis/components.primereact10');\n" +
            "if (Object.keys(api).join(',') !== 'primeReact10UiLibrary') throw new Error('Packed runtime must export only primeReact10UiLibrary.');\n" +
            "if (api.primeReact10UiLibrary.id !== 'cratis-primereact10') throw new Error('Packed manifest did not load with explicit peers.');\n",
    );
    run(process.execPath, ['runtime.mjs'], temporary);

    writeFileSync(
        path.join(temporary, 'fixture.ts'),
        "import { primeReact10UiLibrary } from '@cratis/components.primereact10';\nvoid primeReact10UiLibrary;\n",
    );
    writeFileSync(
