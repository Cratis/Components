...
if (storyFiles.length !== 64) throw new Error(`Expected the existing 64 story modules, found ${storyFiles.length}.`);
...
for (const adapter of inventory.adapters) {
    ...
    if (storyIds.length !== 261 || docsIds.length !== 64) {
        throw new Error(`${adapter.metadata.id} indexed ${storyIds.length} stories and ${docsIds.length} autodocs pages; expected 261 and 64.`);
    }
    ...
    console.log(`${adapter.metadata.id}: 261 stories, 64 autodocs pages, primereact [${expectedVersions.join(', ') || 'none'}].`);
}

const appearances = 2;
const matrixCount = inventory.adapters.length * 261 * appearances;
console.log(`Storybook indexes verified: ${inventory.adapters.length} previews × 261 stable stories × ${appearances} appearances = ${matrixCount} story/appearance cases.`);
