...
const storybookCli = path.join(repositoryRoot, 'node_modules/storybook/dist/bin/dispatcher.js');
...
runNode('Verify adapter inventory', path.join(storybookRoot, 'scripts/verify-adapter-inventory.mjs'));
runNode('Verify PrimeReact peer-major isolation', path.join(storybookRoot, 'scripts/verify-peer-major-isolation.mjs'));
...
for (const adapter of inventory.adapters) {
    runStorybook(`Build ${adapter.metadata.displayName} preview`, [
        'build',
        '--config-dir',
        path.join(storybookRoot, 'preview'),
        '--output-dir',
