...
export const validateAgainstSchema = (candidate, schema) => {
    const resolveReference = (reference) => {
        ...
        if (!isRecord(current))
            throw new Error(
                `Schema reference '${reference}' does not resolve to a rule.`,
            );
        return current;
    };
    ...
    const validate = (value, rule, location = '$') => {
        ...
        if (
            rule.enum &&
            !rule.enum.some((candidateValue) => sameJson(candidateValue, value))
        )
        ...
        if (isRecord(value)) {
            ...
            for (const key of keys) {
                ...
                if (propertyRule)
                    problems.push(
                        ...validate(value[key], propertyRule, `${location}.${key}`),
                    );
        ...
        for (const nested of rule.allOf ?? [])
            problems.push(...validate(value, nested, location));
        if (rule.if && validate(value, rule.if, location).length === 0 && rule.then)
            problems.push(...validate(value, rule.then, location));
        if (rule.oneOf) {
            const matches = rule.oneOf.filter(
                (option) => validate(value, option, location).length === 0,
            ).length;
            if (matches !== 1)
                add(`must match exactly one alternative, matched ${matches}`);
        }
        return problems;
    };
