...
const appearances = requestedAppearance ? [requestedAppearance] : ['baseline-light', 'baseline-dark'];
...
runNode('Pre-matrix peer-major isolation', path.join(storybookRoot, 'scripts/verify-peer-major-isolation.mjs'));
runNode('Pre-matrix Storybook indexes', path.join(storybookRoot, 'scripts/verify-storybook-indexes.mjs'));
...
for (const adapter of inventory.adapters) {
    for (const appearance of appearances) {
        ...
        if (result.error) throw result.error;
        if (result.status !== 0) throw new Error(`Browser matrix failed for ${label} with exit code ${result.status}.`);
    }
}
...
console.log(`Renderer exclusions: ${inventory.exclusions.map(item => `${item.id} (${item.reason})`).join(', ') || 'none'}.`);
