...
describe('when discovering renderer previews', () => {
    ...
    it('should exclude a private adapter workspace without a Core inventory edit', () => {
        ...
        try {
            ...
            writeFileSync(
                path.join(root, 'Source/schemas/ui-adapter.schema.json'),
                JSON.stringify(adapterSchema),
            );
            writeFileSync(
                path.join(root, 'Adapters/Plain/package.json'),
                JSON.stringify({
            ...
            expect(
                inventory.adapters.map((adapter) => adapter.metadata.id),
            ).to.deep.equal(['cratis-built-in']);
