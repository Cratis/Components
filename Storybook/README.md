...
`test-storybook` runs every indexed story with axe in both maintained appearance modes on the built-in preview. The other discovered previews run only the renderer-distinguishing stories: those whose component (or, without a declared component, the story module) transitively imports a production module that calls `unstable_useSlot(...)`, derived from source by `scripts/lib/renderer-matrix-scope.mjs`. The remaining stories render identical DOM on every renderer, so they run once, on the built-in preview. No story is excluded; the run ends by printing the story counts, the matrix scope, and the total number of story/appearance/axe cases. Issue #217 tracks replacing hardcoded evidence counts with generated reviewable inventories. The composed renderer control preserves a stable story id when available, but switching previews remounts the iframe and loses component-local state.
