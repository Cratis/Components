...
import {
    startAnimationLoop as startAnimationLoopExternal,
    updatePositions as updatePositionsExternal,
} from './pivot/animation';
import {
    ANIMATION_SPEED,
    DEFAULT_COLORS,
    type CardSprite,
    type CardColors,
} from './pivot/constants';
...
const asReactMouseEvent = (event: unknown): React.MouseEvent => {
...
export function PivotCanvas<TItem extends object>({
    ...
    }, [grouping, layout, zoomLevel, viewMode, pixiReady, colorOverrides, colorRevision]);
  ...
  */
    ...
    function updateCardContent(sprite: CardSprite, item: TItem) {
        return updateCardContentExternal(
            sprite,
            item,
            selectedId,
            cardWidth,
    ...
    function updatePositions(): boolean {
        return updatePositionsExternal(
            spritesRef.current,
            isViewTransitionRef,
            ANIMATION_SPEED,
        );
