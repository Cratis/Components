...
export function applyFilters(
    ...
    // Every active filter contributes exactly one set to intersect, even when it
    // matches nothing - an empty Uint32Array - so a zero-match filter correctly
    // collapses the overall intersection to empty instead of being skipped (which
    // would let the other active filters' matches leak through unfiltered).
    ...
    for (const filter of filters) {
        if (filter.type === 'categorical' && filter.values && filter.values.size > 0) {
            ...
            sets.push(arrays.length > 0 ? unionSorted(arrays) : new Uint32Array(0));
...
export function computeGrouping(
    ...
    if (field.kind === 'string') {
        return groupByCategorical(
            field,
            visibleIds,
            indexes.categorical.get(groupBy.field),
        );
    } else if (field.kind === 'number') {
...
function groupByNumeric(
    ...
    // A degenerate range (every finite value identical) makes bucketSize zero, which
    // would divide-by-zero into NaN bucket indices below and throw. All finite values
    // equal min in that case, so collapse to a single bucket holding every one of them.
    if (min === max) {
        const ids: number[] = [];
        for (let i = 0; i < visibleIds.length; i++) {
            const id = visibleIds[i];
            if (!isNaN(field.values[id])) {
                ids.push(id);
            }
        }
