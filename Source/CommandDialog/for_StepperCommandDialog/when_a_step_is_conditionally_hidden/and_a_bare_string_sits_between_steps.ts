...
const whitespaceBetweenSteps = '   ';

describe('when a bare string follows the only step and the dialog is on the last rendered step', () => {
    let html: string;

    beforeEach(() => {
        html = renderDialog(panel('Step 1'), whitespaceBetweenSteps);
    });
...
describe('when a bare string sits between two steps and the dialog is on the first rendered step', () => {
    let html: string;

    beforeEach(() => {
        html = renderDialog(panel('Step 1'), whitespaceBetweenSteps, panel('Step 2'));
    });
