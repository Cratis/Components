...
describe('when a bare string follows the only step and the stepper is on the last rendered step', () => {
    let html: string;

    beforeEach(() => {
        html = renderStepper(panel('Step 1'), whitespaceBetweenSteps);
    });
...
describe('when a bare string sits between two steps and the user walks forward', () => {
    let result: { activeStep: number; html: string };

    beforeEach(() => {
        result = walkForward([panel('Step 1'), whitespaceBetweenSteps, panel('Step 2')]);
    });
