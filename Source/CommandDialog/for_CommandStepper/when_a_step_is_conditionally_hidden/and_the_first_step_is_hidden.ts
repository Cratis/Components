...
describe('when the first of two steps is hidden and the stepper is on the last rendered step', () => {
    let html: string;

    beforeEach(() => {
        html = renderStepper(showOptionalStep && panel('Step 1'), panel('Step 2'));
    });
...
describe('when the first of three steps is hidden and the user walks forward', () => {
    let result: { activeStep: number; html: string };

    beforeEach(() => {
        result = walkForward([showOptionalStep && panel('Step 1'), panel('Step 2'), panel('Step 3')]);
    });
