...
describe('when a map produces a single step and the stepper is on the last rendered step', () => {
    let html: string;

    beforeEach(() => {
        html = renderStepper(headersOf(1).map(panel));
    });
...
describe('when a map produces three steps and the user walks forward', () => {
    let result: { activeStep: number; html: string };

    beforeEach(() => {
        result = walkForward(headersOf(3).map(panel));
    });

    it('should_stop_on_the_last_rendered_step', () => {
        result.activeStep.should.equal(2);
    });
