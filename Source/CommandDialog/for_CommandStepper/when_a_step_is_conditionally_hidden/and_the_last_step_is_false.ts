...
describe('when the only hidden step is the last of two and the stepper is on the last rendered step', () => {
    let html: string;

    beforeEach(() => {
        html = renderStepper(panel('Step 1'), showOptionalStep && panel('Step 2'));
    });
...
describe('when the hidden step is the last of three and the user walks forward', () => {
    let result: { activeStep: number; html: string };

    beforeEach(() => {
        result = walkForward([panel('Step 1'), panel('Step 2'), showOptionalStep && panel('Step 3')]);
    });
