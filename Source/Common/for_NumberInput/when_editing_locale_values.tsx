...
describe('when editing locale-aware number values', () => {
    ...
    it('should omit grouping separators when grouping is disabled', async () => {
        const year = await mountNumberInput({
            initialValue: 2026,
            providerLocale: 'nb-NO',
            useGrouping: false,
            'aria-label': 'Year',
        ...
        expect(year.input.value).to.equal('2026');
    });

    it('should keep the locale decimal separator while grouping is disabled', async () => {
        const measurement = await mountNumberInput({
            initialValue: 1234.5,
            providerLocale: 'nb-NO',
            useGrouping: false,
        ...
        expect(measurement.input.value).to.equal('1234,5');
    });

    it('should group by default when grouping is not configured', async () => {
        const grouped = await mountNumberInput({
            initialValue: 2026,
            providerLocale: 'nb-NO',
            'aria-label': 'Year',
        });
        ...
        expect(normalizedSpaces(grouped.input.value)).to.equal('2 026');
    });
});
