...
describe('when client filtering is enabled', () => {
    ...
    it('should render only rows matching the client filter', () => {
        expect(container.querySelectorAll('tbody tr')).to.have.lengthOf(11);
    });

    it('should use the filtered row count instead of the server page count', () => {
        expect(container.querySelector('.cratis-table-paginator')).to.equal(null);
    });
});
