/**
 * Optimized PivotViewer using Pixi.js and Web Worker
 *
 * Drop-in replacement for PivotViewer with high-performance rendering.
 *
 * Architecture:
 * - Web Worker for data processing (filtering, grouping)
 * - Columnar data storage for efficient filtering
 * - Pixi.js canvas rendering for smooth 60fps animations
 * - Modular utilities for animations, selection, and layout
 */
...
export function PivotViewerOptimized<TItem extends object>({
  data,
  dimensions,
  filters,
  defaultDimensionKey,
  ...
  console.log('[PivotViewer] Render', { viewMode });
  ...
  useEffect(() => {
    ...
    engineApplyFilters(currentFilters).then((result) => {
      // Diagnostic: log visible count and sample ids
      try {
        // eslint-disable-next-line no-console
        console.debug('[PivotViewerOptimized] applyFilters result visibleIds.length=', result.visibleIds.length, 'sample=', Array.from(result.visibleIds.slice(0, 10)));
      } catch (e) {
        // ignore
      }
  ...
  const handleCardClick = useCallback((item: TItem, e: MouseEvent, id?: number | string) => {
    ...
    console.log('[PivotViewer] handleCardClick', {
      itemId,
      viewMode,
      layoutViewMode: (layout as any).viewMode, // Check if layout has viewMode info
    ...
    console.log('[PivotViewer] handleCardClick position check', {
      itemId,
      position,
      layoutTotalHeight: layout.totalHeight,
      viewMode
    ...
    console.log('[PivotViewer] handleCardClick start', {
      itemId,
      viewMode,
      layoutTotalHeight: layout.totalHeight,
    ...
    if (viewMode === 'grouped' && cardPosition) {
      ...
      console.log('[PivotViewer] Target Layout Calculation', {
        itemId,
        currentTotalHeight: layout.totalHeight,
        targetTotalHeight,
        targetZoom,
        targetContainerHeight,
        targetLayoutKeys: Object.keys(targetLayout)
      });
