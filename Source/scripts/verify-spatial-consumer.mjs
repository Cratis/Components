...
// --- 4. Strict TypeScript for Canvas with pixi.js installed ---------------------------------------
...
if (tscBin) {
    const fixtureDir = path.join(scratchRoot, 'types-canvas-bundler');
    const { mkdirSync } = await import('node:fs');
    mkdirSync(fixtureDir, { recursive: true });
    writeFileSync(
        path.join(fixtureDir, 'consumer.ts'),
        `import type { CanvasProps, CanvasContext } from '${pkg.name}/Canvas';\ndeclare const props: CanvasProps;\nvoid props;\ndeclare const ctx: CanvasContext;\nvoid ctx;\n`,
    );
    writeFileSync(
    ...
    if (run.status === 0) {
        pass('./Canvas type-checks with pixi.js installed');
    } else {
        fail(
            './Canvas must type-check with pixi.js installed',
...
console.log(
    'All spatial consumer checks passed: Canvas/PivotViewer import with the explicit pixi.js peer, ' +
        'pixi.js resolves to one identical instance with no nested copy, and Canvas type-checks.',
);
