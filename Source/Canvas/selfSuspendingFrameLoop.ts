 ...
 * burning a callback every frame forever on an idle canvas.
 */
...
/**
 * Creates a self-suspending animation-frame loop. The step runs once per frame and returns whether
 * another frame is needed; when it returns `false` the loop suspends until the next {@link SelfSuspendingFrameLoop.wake}.
 * The scheduler is injectable so the suspension logic can be specified without a browser.
 */
