 ...
 */
export const mentionSegments = <TCandidate extends { name: string }>(text: string, candidates: TCandidate[]): MentionSegment<TCandidate>[] => {
    ...
    for (const range of findMentionRanges(text, candidates)) {
        ...
        segments.push({ text: text.slice(range.start, range.end), mention: range.candidate });
        position = range.end;
    }
