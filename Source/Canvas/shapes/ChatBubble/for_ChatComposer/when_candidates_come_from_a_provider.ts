...
import { click, render, typeInto, unmount, type ComposerInTheDom } from './given/a_composer_in_the_dom';
...
describe('when candidates come from a provider', () => {
    ...
    const scout: MentionCandidate = { id: 'agent-1', name: 'Scout', hasAvatar: false, kind: ChatAuthorKind.Agent };
    ...
    beforeEach(async () => {
        ...
        composer = await render(createElement(ChatComposer, {
        ...
        await typeInto(document.querySelector<HTMLTextAreaElement>('.chat-composer__input')!, 'ask @sc');
    });
    ...
    it('should ask the provider with what was typed after the @', () => queriesSeen.should.contain('sc'));

    it('should suggest what the provider answered', () => {
        document.querySelectorAll('.mention-suggestions__item').should.have.lengthOf(1);
        document.querySelector('.mention-suggestions__name')!.textContent!.should.equal('Scout');
    });

    describe('and the suggestion is picked and sent', () => {
        beforeEach(async () => {
            await click(document.querySelector<HTMLButtonElement>('.mention-suggestions__item')!);
            await click(document.querySelector<HTMLButtonElement>('.chat-composer__send')!);
        });

        it('should write the full name into the draft before sending', () => sentText!.should.equal('ask @Scout'));
