...
export const ChatComposer = forwardRef<ChatComposerHandle, ChatComposerProps>(({ mentionCandidates, onSend, autoFocus = false, buildAvatarUrl, labels }, handleRef) => {
    ...
    const choose = (candidate: MentionCandidate) => {
        ...
        requestAnimationFrame(() => inputRef.current?.setSelectionRange(applied.caret, applied.caret));
        inputRef.current?.focus();
    };
    ...
    const handleKeyDown = (event: React.KeyboardEvent<HTMLTextAreaElement>) => {
        if (isSuggesting) {
            ...
            if (event.key === 'Escape') {
                ...
                setDraft(previous => ({ ...previous, caret: previous.text.length }));
                return;
            }
        }
    ...
    return (
        <div className='chat-composer'>
            ...
            <textarea
                ...
                placeholder={labels?.placeholder ?? 'Write a message… (Enter to send)'}
                rows={2}
            />
            <button
                ref={emojiButtonRef}
            ...
            <AnchoredOverlay anchorRef={emojiButtonRef} open={showEmojiPicker} side='above' align='right' gap={6}>
                <ReactionPicker anchorRef={emojiButtonRef} onPick={insertEmoji} onDismiss={() => setShowEmojiPicker(false)} labels={labels?.reactionPicker} />
            </AnchoredOverlay>
        </div>
    );
});
