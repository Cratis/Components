...
The callback transform rewrites structurally-proven single forwarding callbacks on the standalone Components-owned Dropdown and CommandForm field `onValueChange` props:
...
**CommandForm field `onChange` is not a typed prop.** Components 4 CommandForm fields such as `InputTextField`, `CheckboxField`, `DropdownField`, and `MultiSelectField` are Arc `asCommandFormField` wrappers whose public props do not accept `onChange`. Rewriting only the callback leaves an invalid attribute and causes TS2322. The codemod leaves these field handlers unchanged and reports a diagnostic. Move the side effect to the enclosing `CommandForm`'s `onFieldChange(command, fieldName, oldValue, newValue, validationInfo?)` callback. Legacy field `onValueChange` callbacks with a proven event payload are rewritten to semantic values; already-semantic ones are untouched. The standalone `Dropdown` declares `onChange` and can be rewritten.
