...
Use the exact installed Components version and preview each transform before applying it:
...
Never substitute `latest` or a different version if the exact codemod package is unavailable. Stop and migrate manually, keep the application on its current Components package, or wait for the matching publication.
