...
const quiet: ChatTopic = {
    id: 'quiet',
    name: 'Quiet',
    lastActivity: new Date('2026-08-20T10:00:00Z'),
};
const busy: ChatTopic = {
    id: 'busy',
    name: 'Busy',
    lastActivity: new Date('2026-08-26T10:00:00Z'),
};
const justStarted: ChatTopic = {
    id: 'started',
    name: 'Just started',
    started: new Date('2026-08-25T10:00:00Z'),
};
...
describe('when topics arrive in no particular order', () => {
    ...
    it('should fall back to when a topic started', () =>
        ordered[1].should.equal(justStarted));
...
describe('when a date traveled through JSON as a string', () => {
    // SAFETY: exercising a topic whose lastActivity survived a JSON round-trip as a string, not a Date.
    const stringly = {
        id: 'stringly',
        name: 'Stringly',
        lastActivity: '2026-08-27T10:00:00Z' as unknown as Date,
    };
